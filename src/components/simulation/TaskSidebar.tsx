    ...
    >
      ...
      <div className="p-4 border-b border-gray-200 bg-gray-50 sticky top-0 z-10 flex items-center justify-between shrink-0">
        <h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] flex items-center gap-2">
          <BookOpen className="w-3 h-3 text-primary" /> <Translate>{title}</Translate>
        </h2>
        ...
        <Card className="bg-blue-50 border-blue-100 shadow-none overflow-hidden shrink-0">
          <CardContent className="p-4 relative">
            ...
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-blue-700 tracking-wider uppercase"><Translate>Progress</Translate></span>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!isActive && (
                          <p className="text-[10px] text-muted-foreground mt-1 line-clamp-1">
                            <Translate>{task.description}</Translate>
                          </p>
                        )}
                      </div>
                    </div>
                          ...
                          <div className="text-[10px] text-gray-600 space-y-3">
                            ...
                            <div className="space-y-2 bg-blue-50/50 rounded-lg p-3 border border-blue-100">
                              <p className="font-bold text-gray-800 flex items-center gap-2 uppercase tracking-wider text-[9px]">
                                <BookOpen className="w-3 h-3 text-primary" /> <Translate>Directive</Translate>
                              </p>
                              <ul className="space-y-1.5">
                                {task.instructions.map((instruction, idx) => (
                                  <li key={idx} className="flex gap-2">
                                    <span className="w-1 h-1 rounded-full bg-primary mt-1.5 shrink-0" />
                                    <span><Translate>{instruction}</Translate></span>
                                  </li>
                                ))}
                              </ul>
                            </div>
                            ...
                            {task.hint && (
                              <div className="bg-yellow-50 border border-yellow-200 rounded p-2 text-yellow-700">
                                <p className="flex gap-2">
                                  <span className="font-bold"><Translate>💡 HINT:</Translate></span> <span><Translate>{task.hint}</Translate></span>
                                </p>
                              </div>
                            )}
                            ...
                            {task.completed ? (
                              ...
                              >
                                <p className="font-bold flex items-center justify-center gap-1.5 tracking-wider uppercase">
                                  ...
                                  <Translate>Mission Complete</Translate>
                                </p>
                              </motion.div>
                            ) : isActive && onSkipTask && (
                              <div className="mt-3 p-2 border border-amber-200 rounded-lg bg-amber-50">
                                <p className="text-[10px] text-amber-700 mb-2"><Translate>Having trouble? You can skip this task and come back later.</Translate></p>
                                <button
        ...
        {allComplete && (
          <Card className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border-green-500/30 shrink-0">
            <CardContent className="p-4 text-center">
              ...
              >🎉</motion.div>
              <p className="text-foreground font-bold text-sm uppercase tracking-widest"><Translate>Training Complete</Translate></p>
              <p className="text-green-400 text-xs mt-1"><Translate>Ready for the next module!</Translate></p>
            </CardContent>
          </Card>
        )}
      </div>
