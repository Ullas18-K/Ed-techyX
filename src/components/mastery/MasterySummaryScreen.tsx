  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-6xl mx-auto px-3 py-4 flex flex-col gap-3">
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200 }} className="text-center flex-shrink-0">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-1">🎉 Learning Complete!</h1>
        <p className="text-base text-muted-foreground">You've mastered the basics of {currentScenario.topic}</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-2xl p-3 flex-shrink-0">
        <div className="flex items-center gap-1.5 mb-2"><Award className="w-4 h-4 text-primary" /><h3 className="font-bold text-sm text-foreground">Confidence Level</h3></div>
        ...
        <div className="flex justify-between text-xs text-muted-foreground mb-2"><span>Beginner</span><span>Developing</span><span>Confident</span><span>Mastery</span></div>
        <div className="text-center"><span className={cn("text-lg font-bold", mastery.color)}>{mastery.level}</span></div>
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-2xl p-3 flex-shrink-0">
        <div className="flex items-center gap-1.5 mb-2"><BookOpen className="w-4 h-4 text-info" /><h3 className="font-bold text-sm text-foreground">Concepts Covered</h3></div>
        <div className="flex flex-wrap gap-1.5">
          {currentScenario.concepts.map((concept, index) => (
            <span key={index} className="flex items-center gap-1 px-2.5 py-1 glass-card rounded-full text-xs text-success border border-success/30"><span>✓</span><span>{concept}</span></span>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-3 border-warning/20 flex-shrink-0">
        <div className="flex items-center gap-1.5 mb-2"><GraduationCap className="w-4 h-4 text-warning" /><h3 className="font-bold text-sm text-foreground">Exam Connection</h3></div>
        <div className="grid md:grid-cols-3 gap-2">
          <div><p className="text-xs text-muted-foreground mb-0.5">NCERT Chapter</p><p className="text-xs font-semibold text-foreground">{currentScenario.examConnection.ncertChapter}</p></div>
          <div><p className="text-xs text-muted-foreground mb-0.5">Board Relevance</p><p className="text-xs font-semibold text-foreground">{currentScenario.examConnection.boardRelevance}</p></div>
          <div><p className="text-xs text-muted-foreground mb-0.5">Competitive Exams</p><div className="flex flex-wrap gap-0.5">{currentScenario.examConnection.competitiveExams.map((exam, i) => (<span key={i} className="px-1.5 py-0.5 glass-subtle rounded text-xs text-warning font-semibold">{exam}</span>))}</div></div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row gap-2 justify-center flex-shrink-0">
        <Button variant="outline" size="sm" onClick={onRestart} className="gap-1.5 rounded-lg text-xs"><RotateCcw className="w-3 h-3" />Try Similar</Button>
        <Button variant="hero" size="sm" onClick={onNewQuestion} className="gap-1.5 rounded-lg text-xs"><Sparkles className="w-3 h-3" />New Question</Button>
        <Button variant="outline" size="sm" onClick={() => saveToNotes(`Completed learning session on ${currentScenario.topic}`)} className="gap-1.5 rounded-lg text-xs"><BookmarkPlus className="w-3 h-3" />Save Notes</Button>
      </motion.div>
    </motion.div>
  );
}